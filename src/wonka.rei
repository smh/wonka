...
/* Takes a source emitting sources, and creates a sink & source.
...
/* Takes a function and a source, and creates a sink.
   ...
   the source ends.
   Returns an anonymous function that when called will end the stream
   immediately, which propagates an End signal upwards.
 */
let forEach: (
  'a => unit,
  (. (. signalT('a)) => unit) => unit,
...
/* Accepts an observer with properties next and complete, and returns an
   anonymous function that when called will end the stream immediately.
